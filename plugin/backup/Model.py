    ...
    def lookup(self, name):  # FINISH THIS
        ...
        if "/" not in path:
            try:
                ...
                raise (KeyError, "No such dataset %s at %s" % (path, self.get_path()))
            if snapshot:
                try:
                    dset = dset.get_snapshot(snapshot)
                except KeyError:
                    raise (KeyError, "No such snapshot %s at %s" % (snapshot, dset.get_path()))
        else:
            head, tail = path.split("/", 1)
            try:
                child = self.get_child(head)
            except KeyError:
                raise (KeyError, "No such dataset %s at %s" % (head, self.get_path()))
